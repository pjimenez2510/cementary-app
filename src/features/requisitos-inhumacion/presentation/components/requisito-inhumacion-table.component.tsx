import { AlertCircle, Hash, User, MapPin, Calendar, UserCheck, Shield, Pencil, Trash2, Building, Clock, FileText } from "lucide-react";
...
export function RequisitoInhumacionListTable() {
  ...
  const { mutate: deleteRequisitoInhumacion, isPending } = useDeleteRequisitoInhumacionMutation();
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            ...
            {!isLoading && requisitosInhumacion && requisitosInhumacion.length === 0 && (
              <TableRow>
            ...
            {requisitosInhumacion?.map((requisito) => (
              ...
              <TableRow key={requisito.idRequsitoInhumacion}>
                ...
                <TableCell>{requisito.idCementerio?.nombre ?? "Sin cementerio"}</TableCell>
                <TableCell>{requisito.pantoneroACargo}</TableCell>
                <TableCell>{requisito.idSolicitante?.nombres ?? "No hay solicitante"}</TableCell>
                <TableCell>{requisito.idHuecoNicho?.numHueco ??  "Sin nicho"}</TableCell>
                <TableCell>{requisito.idFallecido?.nombres ?? "No hay fallecido"}</TableCell>
                <TableCell>{new Date(requisito.fechaInhumacion).toLocaleDateString()}</TableCell>
                ...
                <TableCell>
                  <div className="flex gap-2">
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>¿Eliminar requisito de inhumación?</AlertDialogTitle>
                          <AlertDialogDescription>
                            Esta acción no se puede deshacer. ¿Deseas eliminar este requisito de inhumación?
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>Cancelar</AlertDialogCancel>
                          <AlertDialogAction
                            onClick={() => deleteRequisitoInhumacion(requisito.idRequsitoInhumacion)}
                            disabled={isPending}
                            className={clsx(
